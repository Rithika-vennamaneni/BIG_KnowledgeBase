...
class HealthcareRAG:
    ...
    def format_answer(self, query: str, results: List[Dict]) -> str:
        """Format search results into a readable answer (retrieval only)"""
        if not results:
            return "❌ No relevant rules found for your query."
        
        answer = f"📝 Query: {query}\n\n"
        answer += f"Found {len(results)} relevant rules:\n"
        answer += "=" * 80 + "\n\n"
        ...
        for i, result in enumerate(results, 1):
            ...
            answer += f"📌 Result {i} (Relevance: {similarity:.2%})\n"
            answer += f"   Payer: {rule['payer_name']}\n"
            answer += f"   Type: {rule['rule_type'].upper()}\n"
            answer += f"   Title: {rule['rule_title']}\n"
            answer += f"   Document: {rule['filename']} (Page {rule['page_number']})\n"
            answer += f"   Content: {rule['rule_content'][:200]}...\n"
            answer += f"   Rule ID: {rule['rule_id']}\n"
            answer += "-" * 80 + "\n\n"
